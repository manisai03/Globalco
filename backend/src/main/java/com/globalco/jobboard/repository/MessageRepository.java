package com.globalco.jobboard.repository;

import com.globalco.jobboard.model.Message;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;

import java.util.List;

public interface MessageRepository extends JpaRepository<Message, Long> {

    @Query("""
        SELECT m FROM Message m
        WHERE (m.sender.id = :userId1 AND m.receiver.id = :userId2)
           OR (m.sender.id = :userId2 AND m.receiver.id = :userId1)
        ORDER BY m.createdAt ASC
        """)
    List<Message> findConversation(Long userId1, Long userId2);

    @Query("""
        SELECT DISTINCT CASE WHEN m.sender.id = :userId THEN m.receiver.id ELSE m.sender.id END
        FROM Message m WHERE m.sender.id = :userId OR m.receiver.id = :userId
        """)
    List<Long> findConversationPartners(Long userId);

    long countByReceiverIdAndReadFlagFalse(Long receiverId);

    @Query("""
        SELECT COUNT(m) FROM Message m
        WHERE m.receiver.id = :userId AND m.sender.id = :partnerId AND m.readFlag = false
        """)
    long countUnreadFromPartner(Long userId, Long partnerId);

    @Query("""
        SELECT COUNT(m) > 0 FROM Message m
        WHERE m.sender.id = :partnerId AND m.receiver.id = :userId
        AND m.sender.role.name = 'ROLE_ADMIN'
        """)
    boolean hasRecruiterMessagedUser(Long userId, Long partnerId);
}
